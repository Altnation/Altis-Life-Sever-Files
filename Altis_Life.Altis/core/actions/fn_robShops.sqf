/*
		...
		Author: MrKraken
		Edited by CWY Elektro and CWY RevZItsDaveo
...
if(side _robber == west) exitWith { hint "You are serving as part of the Altis Police Department! What do you think you are doing?" };
if(side _robber == independent) exitWith { hint "You're a medic! What do you think you are doing?" };
if(_robber distance _shop > 5) exitWith { hint "You need to be within 5m of the cash register to rob it!" };
if (vehicle player != _robber) exitWith { hint "Exit the vehicle in order to rob the gas station." };
...
_rip = true;
_kassa = 35000 + round(random 35000);
...
if(_rip) then
{
   ...
   _marker setMarkerText "ATTENTION! GAS STATION ROBBERY IN PROGRESS!";
